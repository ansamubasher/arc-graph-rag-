...
1b. Use HAS_ALLOCATION specifically when a Party or Property is granted a
    countable resource quantity (e.g. "Tenant shall have 112 parking
    passes"). Prefer HAS_ALLOCATION over APPLIES_TO or REFERENCES for
    these cases — it is more precise and lets these facts be queried
    directly later.
...
[
  ...
  {{"source": "ABC Corp", "relationship": "HAS_ALLOCATION", "target": "Parking Passes"}}
]
...
"""
    ...
    def extract(self, chunk, entities):
        ...
        for r in relationships:
            ...
            if source == target:
                # model occasionally emits a self-relationship despite the
                # prompt rule against it — drop it rather than storing a
            ...
            if relationship not in self.ALLOWED_RELATIONSHIPS or relationship == "OTHER":
                # off-vocabulary (or explicitly unclassifiable) relationship —
                # the prompt tells the model never to use "OTHER" and to omit
                # anything that doesn't fit cleanly, so we honor that here by
                # dropping it rather than forcing it into "OTHER". Silently
                # relabeling would contradict the prompt and let low-confidence
                # edges pollute the graph.
                continue
